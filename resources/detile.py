...
class ImageDeTiler():
    ...
    def __init__(self, searchDir: str = ".", fileType: str = "tif"):
        self._dir = None
        ...
        if fileType.lower() not in ImageDeTiler._getAllowedImageTypes():
            raise ValueError(
                f"Invalid image type `{fileType.lower()}`, must be one of {ImageDeTiler._getAllowedImageTypes()}")
    ...
    def workingDir(self, newDir: str):
        ...
        self.folders = frozenset(
            [int(x) for x in os.listdir(self._dir) if not x.startswith(".")])
        xCols = None
        for yDir in self.folders:
            xCheckDir = os.path.join(self._dir, str(yDir))
            if xCols is None:
                xCols = frozenset([int(os.path.basename(x).replace(f".{self.fileType}", "")) for x in glob(
                    os.path.join(xCheckDir, f"*.{self.fileType}"))])
                self._xRange = (min(xCols), max(xCols))
                if frozenset([x for x in range(self._xRange[0], self._xRange[1] + 1)]) != xCols:
                    raise FileNotFoundError(
                        f"The smallest X tile is {self._xRange[0]} and the largest {self._xRange[1]}; every value in-between must be supplied.")
                continue
            matchCols = frozenset([int(os.path.basename(x).replace(
                f".{self.fileType}", "")) for x in glob(os.path.join(xCheckDir, f"*.{self.fileType}"))])
            if not matchCols == xCols:
                raise FileNotFoundError(
                    f"Invalid directory structure: all child directories should have identical filenames in the format <xColumnNumber>.{self.fileType}")
    ...
    def folders(self, yColumns: Iterable):
        ...
        if frozenset([x for x in range(self._yRange[0], self._yRange[1] + 1)]) != yCols:
            raise FileNotFoundError(
                f"The smallest Y tile is {self._yRange[0]} and the largest {self._yRange[1]}; every value in-between must be supplied.")
    ...
    def process(self, outputName: str = "output", overrideOutputDir: Optional[str] = None, debug: bool = False) -> str:
        ...
        if debug:
            print("----------------------------------------------")
            print(
                f"Preparing output image `{outputPath}` at resolution {self.finalWidth}x{self.finalHeight}")
            print("----------------------------------------------")
        builder = Image.new(self._getOutputFileColorspace(),
    ...
    def getCompositedXTiles(self, path: str, debug: bool = False) -> Image.Image:
        ...
        """
        result = Image.new(self._getOutputFileColorspace(),
                           (self.finalWidth, self.getYResolution()))
